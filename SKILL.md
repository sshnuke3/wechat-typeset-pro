...
description: "微信公众号模板化排版引擎：Markdown → 微信兼容内联样式 HTML。内置 30 套预设主题 + 可视化画廊预览选择 + AI 内容结构增强 + 深色模式 + 代码高亮，可选一键推送到公众号草稿箱。当用户要求用「现成主题/模板」排版、需要预览多套风格再挑一套、或需要推送草稿箱时使用；若用户要的是「为这篇文章现场定制设计」，改用 wechat-typesetting-cy。"
---
